...
Works on Meta Quest 1, ... Quest 3, Phones and other devices with browsers.
...
Click "I am HOST".
Enter a unique Room Name (e.g., my-room-123). The app will tell you if it is available.
Click Create Room.
...
Click Load Video. The video will load on your screen and wait for guests.
2. The Guests (The Viewers)
Open the same app link on their device (Quest, Phone, etc.).
Click "I am GUEST".
Enter the Room Code provided by the Host.
Click Connect.
The video will automatically load. When the Host presses play, everyone watches together.
